    ...
    abstract class TimelineTrackBaseGUI : TreeViewItem, IBounds
    {
        ...
        protected void DrawTrackState(Rect trackRect, Rect expandedRect, TrackAsset track)
        {
            if (Event.current.type == EventType.Layout)
            {
                ...
                {
                    Rect bgRect = expandedRect;
                    TimelineWindow.instance.OverlayDrawData.Add(TimelineWindow.OverlayData.CreateTextureOverlay(GUIClip.Unclip(bgRect), DirectorStyles.Instance.lockedBG.normal.background));
                    needStateBox = true;
                }
